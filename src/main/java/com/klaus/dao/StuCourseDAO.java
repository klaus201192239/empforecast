...
public interface StuCourseDAO {
	...
	@Select("select distinct stuid from stucourse where stuid<#{stuId} order by stuid limit 10;")
    public List<String> getStudentCourseLimit10(@Param("stuId") String stuId);
	
	@Select("select distinct stuid from stucourse order by stuid limit 10")
